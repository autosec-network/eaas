...
CREATE UNIQUE INDEX `case_insensitive_name` ON `api_keys` (lower("name"));--> statement-breakpoint
CREATE TABLE `datakeys` (
	...
	`bw_pub_id` blob,
	`bw_pub_id_utf8` text GENERATED ALWAYS AS (lower(format('%s-%s-%s-%s-%s', substr(hex("bw_pub_id"),1,8), substr(hex("bw_pub_id"),9,4), substr(hex("bw_pub_id"),13,4), substr(hex("bw_pub_id"),17,4), substr(hex("bw_pub_id"),21)))) VIRTUAL,
	`bw_pri_id` blob NOT NULL,
	`bw_pri_id_utf8` text GENERATED ALWAYS AS (lower(format('%s-%s-%s-%s-%s', substr(hex("bw_pri_id"),1,8), substr(hex("bw_pri_id"),9,4), substr(hex("bw_pri_id"),13,4), substr(hex("bw_pri_id"),17,4), substr(hex("bw_pri_id"),21)))) VIRTUAL,
	`a_time` text(24),
	`b_time` text(24) DEFAULT (strftime('%FT%H:%M:%fZ', CURRENT_TIMESTAMP)) NOT NULL,
	`encryption_count` blob DEFAULT (unhex(0)) NOT NULL,
	FOREIGN KEY (`kr_id`) REFERENCES `keyrings`(`kr_id`) ON UPDATE cascade ON DELETE cascade
);
--> statement-breakpoint
CREATE UNIQUE INDEX `datakeys_bw_pub_id_unique` ON `datakeys` (`bw_pub_id`);--> statement-breakpoint
CREATE UNIQUE INDEX `datakeys_bw_pri_id_unique` ON `datakeys` (`bw_pri_id`);--> statement-breakpoint
CREATE TABLE `keyrings` (
	`kr_id` blob PRIMARY KEY NOT NULL,
	`kr_id_utf8` text GENERATED ALWAYS AS (lower(format('%s-%s-%s-%s-%s', substr(hex("kr_id"),1,8), substr(hex("kr_id"),9,4), substr(hex("kr_id"),13,4), substr(hex("kr_id"),17,4), substr(hex("kr_id"),21)))) VIRTUAL,
	`name` text NOT NULL,
	`exportable` integer DEFAULT false NOT NULL,
	`key_type` text NOT NULL,
	`key_size` integer,
	`hash_size` integer,
	`time_rotation` integer DEFAULT true NOT NULL,
	`count_rotation` blob DEFAULT (unhex(100000000)),
	`b_time` text(24) DEFAULT (strftime('%FT%H:%M:%fZ', CURRENT_TIMESTAMP)) NOT NULL,
	`c_time` text(24) DEFAULT (strftime('%FT%H:%M:%fZ', CURRENT_TIMESTAMP)) NOT NULL,
	`m_time` text(24) DEFAULT (strftime('%FT%H:%M:%fZ', CURRENT_TIMESTAMP)) NOT NULL
);
--> statement-breakpoint
CREATE UNIQUE INDEX `case_insensitive_name` ON `keyrings` (lower("name"));--> statement-breakpoint
CREATE TABLE `keyrings_api_keys` (
	`kr_id` blob NOT NULL,
	`kr_id_utf8` text GENERATED ALWAYS AS (lower(format('%s-%s-%s-%s-%s', substr(hex("kr_id"),1,8), substr(hex("kr_id"),9,4), substr(hex("kr_id"),13,4), substr(hex("kr_id"),17,4), substr(hex("kr_id"),21)))) VIRTUAL,
	`ak_id` blob NOT NULL,
	`ak_id_utf8` text GENERATED ALWAYS AS (lower(format('%s-%s-%s-%s-%s', substr(hex("ak_id"),1,8), substr(hex("ak_id"),9,4), substr(hex("ak_id"),13,4), substr(hex("ak_id"),17,4), substr(hex("ak_id"),21)))) VIRTUAL,
...
--> statement-breakpoint
CREATE UNIQUE INDEX `keyrings_api_keys_kr_id_ak_id_unique` ON `keyrings_api_keys` (`kr_id`,`ak_id`);--> statement-breakpoint
