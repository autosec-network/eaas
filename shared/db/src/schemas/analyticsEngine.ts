...
export const EAAS_LANG_ANALYTICS = sqliteTable('EAAS_LANG_ANALYTICS', (ela) => ({
	...
	timestamp: ela.text({ mode: 'text', length: 19 }).$type<`${number}-${number}-${number} ${number}:${number}:${number}`>(),
...
export const EAAS_PLATFORM_ANALYTICS = sqliteTable('EAAS_PLATFORM_ANALYTICS', (epa) => ({
	...
	timestamp: epa.text({ mode: 'text', length: 19 }).$type<`${number}-${number}-${number} ${number}:${number}:${number}`>(),
	/**
