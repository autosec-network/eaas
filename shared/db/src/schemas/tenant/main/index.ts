...
import { primaryKey, snakeCase, uniqueIndex } from 'drizzle-orm/sqlite-core';
...
export const keyrings = snakeCase.table(
	...
	(k) => [uniqueIndex('case_insensitive_keyring_name').on(sql<string>`lower(${k.name})`)],
);

export const users_keyrings = snakeCase.table('users_keyrings', (uk) => ({
	u_id: uk
		.blob({ mode: 'buffer' })
		.notNull()
...
export const datakeys = snakeCase.table('datakeys', (d) => ({
 ...
 */
export const alarms = snakeCase.table('alarms', (a) => ({
	/**
