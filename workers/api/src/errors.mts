...
export function serializeError(error: unknown): ProblemErrorEntry {
	if (error instanceof AggregateError) {
		return {
			...
			...(error.cause != null && { cause: String(error.cause) }),
...
const problemErrorObjectSchema: z.ZodType<ProblemErrorObject> = z.object({
	name: z.string(),
	message: z.string(),
	cause: z.string().optional(),
});

/** Recursive error schema covering both simple errors and AggregateErrors */
const problemErrorEntrySchema: z.ZodType<ProblemErrorEntry> = z.union([
	problemErrorObjectSchema,
	z.object({
		message: z.string(),
		cause: z.string().optional(),
		errors: z.array(z.lazy(() => problemErrorEntrySchema)),
	}),
]);
...
export const problemDetailsSchema = z
	.object({
		type: z.string().url().openapi({ example: 'https://api.eaas.autosec.network/#tag/api-key-management/DELETE/apikeys/{token_id}' }),
		status: z.int().openapi({ example: 400 }),
		...
		errors: z.array(problemErrorEntrySchema).optional(),
	})
