...
app.openapi(route, async (c) => {
	...
	return Promise.all([
		...
		wrapTime(
			c,
			'r_db-delete-apikey',
			c.var.r_db.delete(rootSchema.api_keys_tenants).where(
				and(
					eq(rootSchema.api_keys_tenants.ak_id, sql<Buffer>`unhex(${ak_id.hex})`),
					// Root delete is scoped to the caller's own tenant - `ak_id` alone is the root primary key, so without the `t_id` predicate this would delete *any* tenant's mapping for a guessed/observed `ak_id`, orphaning their key. Check that scoped delete actually hit a row (via `meta.changes`, available on the D1-backed `r_db`) before touching the tenant DO at all, so a foreign `token_id` can't be used to probe or disturb `t_db`.
					eq(rootSchema.api_keys_tenants.t_id, sql<Buffer>`unhex(${c.var.t_id.hex})`),
				),
			),
			undefined,
			3,
		...
		.then(() => {
			...
			// Since we can't reliably check the changes count across different database types, we'll return success. The 404 case would only happen if both databases failed to find the record, but since we're using the same ak_id for both, it's unlikely one would exist without the other.
			return c.json(
				{
					success: true,
