...
import { BitwardenCloudEndpoints } from '~do/BitwardenSession.mjs';
	...
	override async fetch(request: Request) {
		...
		app.use('/test', async (c, next) => {
			if (c.env.NODE_ENV === 'development') {
				const doId = c.env.BITWARDEN_SESSION.newUniqueId();
				const doStub = c.env.BITWARDEN_SESSION.get(doId);
				await doStub.init({ endpoints: { base: BitwardenCloudEndpoints.Api.us, authentication: BitwardenCloudEndpoints.Identity.us } });
				await doStub.auth(c.env.US_BW_SM_ACCESS_TOKEN);
				const projects = await doStub.getProjects();
				c.executionCtx.waitUntil(doStub.nuke('delete test'));
