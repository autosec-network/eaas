	...
	async fetch(request, env, ctx) {
		...
		app.use('*', async (c, next) => {
			...
			if (c.req.raw.headers.has('X-Timestamp-S') && c.req.raw.headers.has('X-Timestamp-MS')) {
				c.set('requestDateTime', new Date(parseInt(request.headers.get('X-Timestamp-S')!, 10) * 1000 + parseInt(request.headers.get('X-Timestamp-MS')!, 10)));
			} else if ((cf.clientTcpRtt ?? (cf['clientQuicRtt'] as number | undefined) ?? 0) > 0) {
				c.set('requestDateTime', new Date(Date.now() - (cf.clientTcpRtt ?? (cf['clientQuicRtt'] as number | undefined) ?? 0)));
			} else {
				c.set('requestDateTime', new Date());
			}
