				...
				.then(async ([row]) => {
					...
					if (row) {
						...
						if (expired && failExpire) {
							...
							c.set('t_jurisdiction', row.jurisdiction);
							c.set(
								't_db',
								await import('db').then(async ({ drizzleD0 }) => {
									const doId = row.jurisdiction ? c.env.TENANT_D0.jurisdiction(row.jurisdiction).idFromString(row.do_id) : c.env.TENANT_D0.idFromString(row.do_id);

									return drizzleD0(c.env.TENANT_D0.get(doId), {
										...(c.env.NODE_ENV !== 'production' && { logger: await import('drizzle-orm/logger').then(async ({ DefaultLogger }) => new DefaultLogger({ writer: await import('db').then(({ DebugLogWriter }) => new DebugLogWriter(row.do_id)) })) }),
										schema: await import('db/schemas/tenant/main'),
							...
							return Promise.all([import('db/schemas/tenant/main'), import('drizzle-orm/sql')])
								...
								.then(async (hashCheck) => {
									...
									if (!expired && hashCheck) {
										await Promise.all([import('db/schemas/tenant/main'), import('drizzle-orm/sql')])
