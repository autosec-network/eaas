				...
				.then(async ([row]) => {
					...
					if (row) {
						...
						if (expired && failExpire) {
							...
							c.set(
								...
								await import('db').then(async ({ drizzleD0 }) =>
									drizzleD0(c.env.TENANT_D0.getByName(row.do_id.hex), {
										...
										schema: await import('~do/TenantD0/db/index.js'),
										casing: 'snake_case',
							...
							return Promise.all([import('~do/TenantD0/db/index.js'), import('drizzle-orm/sql')])
								...
								.then(async (hashCheck) => {
									...
									if (!expired && hashCheck) {
										await Promise.all([import('~do/TenantD0/db/index.js'), import('drizzle-orm/sql')])
