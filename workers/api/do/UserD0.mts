...
import { UserPropertiesSchema } from 'db';
...
const SYSTEM_ALARMS: Record<UUID, { when: Parameters<UserD0['schedule']>[0]; callee: Parameters<UserD0['schedule']>[1]; payload?: Parameters<UserD0['schedule']>[2] }> = {
...
export class UserD0 extends BaseD0 {
	...
	private async _setupSystemAlarms() {
		const systemAlarmEntries = Object.entries(SYSTEM_ALARMS) as [UUID, (typeof SYSTEM_ALARMS)[UUID]][];
		...
		if (systemAlarmEntries.length > 0) {
			...
			const missingSystemAlarms = systemAlarmEntries.filter(([id]) => !existingSystemAlarmIds.has(id)).map(([id, alarm]) => this.schedule(alarm.when, alarm.callee, alarm.payload, id));
	...
	>(when: Date | number | string[], callee: MethodNames<UserD0>, payload?: T, id?: UUID) {
		...
		if (when instanceof Date) {
			...
			await this.drizzle.insert(userSchema.alarms).values({
				...
				type: 'cron',
				cron: when,
				next_time: nextExecutionTimeWithJitter.toISOString(),
			});
				...
				and(
					...
					...(criteria.timeRange
						? [
								...
								gte(userSchema.alarms.next_time, (criteria.timeRange.start ?? new Date(0)).toISOString()),
								// Until end date or max date
								lte(userSchema.alarms.next_time, (criteria.timeRange.end ?? new Date(Number(BigInt('0x0fffffffffff')))).toISOString()),
							]
						: []),
				),
			)
	...
	override async alarm() {
		...
		const rows = await this.drizzle
			...
			.from(userSchema.alarms)
			.where(lte(userSchema.alarms.next_time, now.toISOString()));
	...
	public _cleanupVerificationTokens() {
		this.ctx.waitUntil(this.drizzle.delete(userSchema.auth_verification_token).where(lte(userSchema.auth_verification_token.expires_timestamp, new Date().toISOString())));
	}
	...
	public updateProperties(_properties: Partial<zm.input<typeof UserPropertiesSchema>>, background: boolean = true, lazy: boolean = true): Promise<Partial<zm.output<typeof UserPropertiesSchema>>> {
		return zm
	...
	public updatePropertiesSync(_properties: Partial<zm.input<typeof UserPropertiesSchema>>, background: boolean = true, lazy: boolean = true): Partial<zm.output<typeof UserPropertiesSchema>> {
