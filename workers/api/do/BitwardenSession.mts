...
export class BitwardenSession extends DurableObject<EnvVars> {
			...
			async function* fetchSecretPages() {
				...
				do {
					...
					if (response.ok) {
						...
						throw new Error('Failed to get secrets', { cause: await response.text() });
					}
	...
	public async decryptSecret(accessToken: string, cipherText: string, iv?: boolean) {
		...
		// Step 2: Create the SymmetricCryptoKey
		const symmetricKey = await SymmetricCryptoKey.fromBase64Key(await this.getOrgEncryptionKey(accessToken), encString.encType as 0 | 1 | 2);
	...
	public async encryptSecret(accessToken: string, plainText: string, iv?: Buffer, version: 0 | 1 | 2 = 2) {
		...
		const symmetricKey = await SymmetricCryptoKey.fromBase64Key(await this.getOrgEncryptionKey(accessToken), version);

		// Step 2: Parse the string into an EncString object
		const encString = await EncString.encryptAes256Hmac(Buffer.from(plainText, 'utf8'), symmetricKey, iv);
