...
export namespace BitwardenCloudEndpoints {
	export enum Identity {
		us = 'https://identity.bitwarden.com/connect/token',
...
export class BitwardenSession extends DurableObject<EnvVars> {
		...
		if (identityEndpoint) {
			...
			if (response.ok) {
				...
				await Promise.all([
					...
					identitySave,
					this.ctx.storage.put('jwt', jwt, { allowConcurrency: true }),
				]);
	...
	public async getProjects() {
		const [orgId, { apiEndpoint, accessToken }] = await Promise.all([
			// Separate for typing reasons
			this.ctx.storage.get<ParsedJwt>('jwt', { allowConcurrency: true }).then((jwt) => jwt?.organization),
			this.ctx.storage.get<string>(['apiEndpoint', 'accessToken'], { allowConcurrency: true }).then((results) => Object.fromEntries(results.entries())),
		]);

		if (apiEndpoint && orgId && accessToken) {
			...
			if (!accessToken) errors.push(new Error('Access token not found'));
