	...
	public async auth(accessToken: string) {
		...
		if (identityEndpoint) {
			...
			if (response.ok) {
				...
				throw new Error(`${response.status} Failed to get token: ${await response.text()}`);
			}
		} else {
	...
	public async getProjects() {
		...
		if (apiEndpoint && orgId && jwt) {
			...
			if (response.ok) {
				...
				throw new Error(`${response.status} Failed to get projects: ${await response.text()}`);
			}
		} else {
			const errors: Error[] = [];
	...
	public async getSecretsAndProjects() {
		...
		if (apiEndpoint && orgId && jwt) {
			...
			if (response.ok) {
				...
				throw new Error(`${response.status} Failed to get secrets and projects: ${await response.text()}`);
			}
		} else {
			const errors: Error[] = [];
			if (!apiEndpoint) errors.push(new Error('API endpoint not found'));
			...
			async function* fetchSecretPages() {
				...
				do {
					...
					if (response.ok) {
						...
						throw new Error(`${response.status} Failed to get secrets: ${await response.text()}`);
					}
	...
	public async setSecret(_options: zm.input<typeof BitwardenSession.setSecretOptions>) {
		...
		if (apiEndpoint && orgId && jwt) {
			...
			if (response.ok) {
				...
				throw new Error(`${response.status} Failed to save secret: ${await response.text()}`);
			}
		} else {
			const errors: Error[] = [];
			if (!apiEndpoint) errors.push(new Error('API endpoint not found'));
