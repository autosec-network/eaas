...
import { TenantPropertiesSchema } from 'db';
...
const SYSTEM_ALARMS: Record<UUID, { when: Parameters<TenantD0['schedule']>[0]; callee: Parameters<TenantD0['schedule']>[1]; payload?: Parameters<TenantD0['schedule']>[2] }> = {} as const;
...
export class TenantD0 extends BaseD0 {
	...
	private async _scheduleNextAlarm() {
		const [alarm] = await this.drizzle
			...
			.from(tenantSchema.alarms)
			.where(gt(tenantSchema.alarms.next_time, new Date().toISOString()))
				...
				and(
					...
					...(criteria.timeRange
						? [
								...
								gte(tenantSchema.alarms.next_time, (criteria.timeRange.start ?? new Date(0)).toISOString()),
								// Until end date or max date
								lte(tenantSchema.alarms.next_time, (criteria.timeRange.end ?? new Date(Number(BigInt('0x0fffffffffff')))).toISOString()),
							]
						: []),
				),
			)
	...
	override async alarm() {
		...
		const rows = await this.drizzle
			...
			.from(tenantSchema.alarms)
			.where(lte(tenantSchema.alarms.next_time, now.toISOString()));
	...
	public updateProperties(_properties: Partial<zm.input<typeof TenantPropertiesSchema>>, background: boolean = true, lazy: boolean = true): Promise<Partial<zm.output<typeof TenantPropertiesSchema>>> {
		return zm
	...
	public updatePropertiesSync(_properties: Partial<zm.input<typeof TenantPropertiesSchema>>, background: boolean = true, lazy: boolean = true): Partial<zm.output<typeof TenantPropertiesSchema>> {
