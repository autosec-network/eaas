...
- **Entry** `src/index.ts` — a `WorkerEntrypoint`. Its `fetch` builds the Hono app per-request, wires middleware, and mounts `src/base`. It also **re-exports every Durable Object and Workflow class** (workerd only discovers them from the `main` file / `wrangler.jsonc`).
