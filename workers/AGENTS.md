...
The rejection comes from `ctx.abort()`, so it's specific to a **hard** nuke: `nuke(reason, false)` wipes storage without aborting and resolves normally. Teardowns that need to inspect their own outcome prefer the soft form for exactly that reason — `TenantD0.purge()` uses it throughout and resolves with a summary, so a rejection from it is a genuine failure with nothing to filter.

`admin` has a shared `isNukedError()` helper for the hard case in `admin/src/routes/[environment]/tenants/tenant-ops.ts` — reuse it (import across route files, as `user-ops.ts` and the sessions route already do) rather than re-deriving the check. Fixed instances: `purgeUser` in `user-ops.ts`, `useNukeOrphanedDo` in `tenants/index.tsx`, `useEndSessions` in `users/[uid]/sessions/index.tsx`. This applies wherever `.nuke()` is called across workers (`admin`, `customer`, `api`, `do-proxy`), not just those call sites — `customer` and `api` currently fire nukes off via `ctx.waitUntil()` without inspecting the result, which sidesteps this specific bug but still logs a spurious rejection on every successful nuke.
