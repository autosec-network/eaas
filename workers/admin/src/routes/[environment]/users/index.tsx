...
type UserDoPlacement = 'none' | `jurisdiction:${DurableObjectJurisdiction}`;

const getEnumEntries = <TValue extends string>(enumObject: Record<string, TValue>): [string, TValue][] => Object.entries(enumObject);

const DO_JURISDICTION_OPTIONS = getEnumEntries(DOJurisdictions as Record<string, DurableObjectJurisdiction>);
...
const parseUserDoPlacement = (placement: string): { kind: 'none' } | { kind: 'jurisdiction'; value: DurableObjectJurisdiction } => {
	if (placement === 'none') {
		return { kind: 'none' };
	}
...
export const useDurableObjects = routeLoader$(async ({ platform }) => async () => {
	...
	for await (const instance of cf.durableObjects.namespaces.objects.list(StaticDatabase.User['eaas-api-prod_UserD0'], {
...
export const useAssignTenant = routeAction$(
	...
	zod$({ userIds: z.array(z.string()), tenantId: z.string().min(1) }),
);
...
export const useLoadEmails = routeAction$(
	async (data, { sharedMap, fail }) => {
		...
		for (const uidHex of data.userIds) {
			...
			if ('failed' in rows) return rows;
			results[uidHex] = rows[0]?.email_key ?? 'N/A';
		}
...
export default component$(() => {
	...
	const handleLoadEmails = $(async () => {
		...
		const result = await loadEmailsAction.submit({ userIds: ids });
	...
	return (
		<section class="mx-auto max-w-7xl px-4 py-6">
			...
			<div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
				<h1 class="text-heading text-2xl font-bold dark:text-white">
					Users
					<span class="text-body-subtle ml-2 text-base font-normal dark:text-gray-400">({loc.params['environment']})</span>
				</h1>
