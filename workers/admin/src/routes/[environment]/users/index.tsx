	...
	async (data, { sharedMap, fail, platform }) => {
		...
		for (const uidHex of data.userIds) {
			...
			if (!('failed' in results)) {
				...
				// If DO was instanciated
				if (result?.do_id) {
					const doNamespace = platform.env.USER_D0_PROD;
					const doId = result.jurisdiction ? doNamespace.jurisdiction(result.jurisdiction).idFromString(result.do_id) : doNamespace.idFromString(result.do_id);
