...
export const useUserDetail = routeLoader$(async ({ sharedMap, params, platform }) => {
	...
	const u_db = sharedMap.get('u_db') as SqliteRemoteDatabase<typeof userSchema>;
	...
	return async () => {
		...
		let alarms: { id: string; callee: string; payload: unknown[]; type: 'scheduled' | 'delayed' | 'cron'; next_time: string; delay_in_seconds: number | null; cron: string[] | null }[] = [];
		...
		const sessions = await Promise.all(
			sessionRows.map(async (row) => {
				const sessionDoId = sessionNamespace.idFromString(row.session_token);
			...
			rows.sort((left, right) => {
				...
				return Date.parse(right.expires) - Date.parse(left.expires);
			}),
		);
	...
	async (data, { platform, sharedMap, fail }) => {
		...
		for (const tokenHex of data.sessionTokens) {
			...
			await sessionNamespace
				.get(sessionNamespace.idFromString(tokenHex))
				.nuke('Ended by admin')
				.then(() =>
					r_db
...
export const head: DocumentHead = {
	title: 'User Detail — Sushidata Admin',
};
