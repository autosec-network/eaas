	...
	return (
		<nav class="border-default-medium flex flex-wrap gap-1 border-b dark:border-gray-700" aria-label="Tenant sections">
			{TENANT_TABS.map((tab) => {
				...
				return (
					<Link prefetch="js" key={tab.segment} href={href} aria-current={active ? 'page' : undefined} class={['-mb-px border-b-2 px-4 py-2 text-sm font-medium', active ? 'border-primary-accent text-primary-accent' : 'text-body-subtle hover:text-heading border-transparent dark:text-gray-400 dark:hover:text-gray-200'].join(' ')}>
						{tab.label}
					</Link>
				);
			})}
