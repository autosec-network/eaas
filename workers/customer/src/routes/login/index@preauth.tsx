	...
	useVisibleTask$(({ track, cleanup }) => {
		...
		const onInput = () => {
			...
			emailFieldDebouncer.value = window.setTimeout(() => {
				emailValid.value = zm.email({ pattern: zm.regexes.idnEmail }).safeParse(emailField.value?.value).success;
			}, 300);
		};
					...
					<div class="border-surface-light/60 shadow-primary-accent/5 dark:border-surface-dark/60 dark:bg-surface-dark/70 rounded-2xl border bg-white/70 p-8 shadow-xl backdrop-blur-md">
						...
						{showLogin.value ? (
							<div class="space-y-3">
								...
								{/* Email magic link */}
								{emailSent.value ? (
									<div class="rounded-lg border border-green-200 bg-green-50 p-3 text-sm text-green-700 dark:border-green-800 dark:bg-green-900/30 dark:text-green-300">Check your email for a magic link to sign in.</div>
								) : (
									<>
										...
										<div class="flex gap-2">
											<input
												...
												class="border-surface-light/60 dark:border-surface-dark/60 dark:bg-surface-dark/50 flex-1 rounded-xl border px-4 py-3 text-sm text-gray-900 placeholder-gray-400 outline-none focus:border-blue-400 focus:ring-1 focus:ring-blue-400 disabled:opacity-60 dark:text-white dark:placeholder-gray-500"
											/>
											<button
												type="button"
												...
												onClick$={$(async (event: Event) => {
													...
													try {
														...
														if (typeof successOrErrors === 'boolean') {
															...
															await signIn.submit({ providerId: 'email', options: { email: emailField.value!.value, callbackUrl: '/' } });
														...
														emailError.value = error instanceof Error ? error.message : 'Failed to send magic link.';
