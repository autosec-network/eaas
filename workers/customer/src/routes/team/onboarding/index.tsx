...
const getProjects = server$(async function (jurisdiction: DOJurisdictions | null, baseEndpoint: string, authEndpoint: string, apiKey: string) {
	...
	try {
		await doStub.init({ endpoints: { base: baseEndpoint, authentication: authEndpoint } });
				...
				<Form action={action} class="space-y-5">
					...
					<div class="flex flex-col gap-4 md:flex-row md:items-end">
						<div class="flex items-center justify-center md:mb-1">
							<div class="border-primary-accent/20 bg-primary-accent/10 text-primary-accent flex h-14 w-14 items-center justify-center overflow-hidden rounded-full border text-lg font-semibold">{avatarPreview.value ? <img src={encodeURIComponent(`/image/proxy?url=${avatarPreview.value}`)} width={56} height={56} alt="Team avatar preview" class="h-full w-full object-cover" /> : <span>{teamNamePreview.value.trim().charAt(0).toUpperCase() || '?'}</span>}</div>
						</div>
						<div class="flex-1">
						...
						<div class="flex-1">
							...
							<input
								...
								onInput$={(_, el) => {
									avatarPreview.value = el.value.startsWith('https://') ? el.value : '';
								}}
							/>
						</div>
					</div>
