...
const useApiKeys = routeLoader$(({ sharedMap, resolveValue }) => async () => {
	...
	if (perms && perms.r_apikeys >= Permissions.Read) {
		const t_db = sharedMap.get('t_db') as SqliteRemoteDatabase<typeof tenantSchema>;
...
const useCreateApiKey = routeAction$(
	async (data, { sharedMap, fail }) => {
		...
		const t_db = sharedMap.get('t_db') as SqliteRemoteDatabase<typeof tenantSchema>;
		const now = new Date();
		const akIdHex = uuidv7().replaceAll('-', '').toLowerCase();
		const akIdBase64url = Buffer.from(akIdHex, 'hex').toString('base64url');
		const secret = randomBytes(32);
		const hashHex = createHash('sha256').update(secret).digest('hex');
		const secretBase64url = secret.toString('base64url');
		const token = `0.${akIdBase64url}.${secretBase64url}`;
		const expires = new Date(data.expires);

		await t_db.insert(tenantSchema.api_keys).values({
			ak_id: sql`unhex(${akIdHex})`,
			name: data.name,
			hash: sql`unhex(${hashHex})`,
		...
		return {
			success: true,
			token,
			ak_id_base64url: akIdBase64url,
		};
	},
	zod$(
		z.object({
			name: z.string().trim().nonempty(),
			expires: z.string().trim().nonempty().datetime(),
...
const useRotateApiKey = routeAction$(
	async (data, { sharedMap, fail }) => {
		...
		const hashHex = createHash('sha256').update(secret).digest('hex');
		const secretBase64url = secret.toString('base64url');
		const token = `0.${data.ak_id_base64url}.${secretBase64url}`;
		...
		await t_db
			...
			.where(eq(tenantSchema.api_keys.ak_id, sql`unhex(${Buffer.from(data.ak_id_base64url, 'base64url').toString('hex')})`));
		...
		<div class="mx-auto w-full max-w-7xl px-6 py-10">
			...
			{createApiKey.value?.success && createApiKey.value.token ? (
				<div class="mb-6 rounded-xl border border-amber-300 bg-amber-50 p-4 text-sm text-amber-900 dark:border-amber-800 dark:bg-amber-950/40 dark:text-amber-200">
					<p class="mb-2 font-semibold">{m.apikeys_banner_new_title()}</p>
									...
									<li key={row.ak_id.base64} class="p-4">
										...
										<div class="flex flex-col gap-2 md:flex-row md:items-end md:gap-3">
											{userPermissions.value && userPermissions.value.r_apikeys >= Permissions.Write ? (
												<Form
													action={editApiKey}
													class="flex flex-1 flex-col gap-2 md:flex-row md:items-end md:gap-2"
													onSubmit$={(_, form) => {
													...
													<label class="flex-1 text-xs text-gray-600 dark:text-gray-300">
														...
														<input name="expires" type="datetime-local" required class={fieldClass} value={new Date(row.expires).toISOString().slice(0, 16)} />
													</label>
