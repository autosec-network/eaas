 ...
 */
const readVaultConnection = async (platform: QwikCityPlatform, r_db: DrizzleD1Database, t_do: ReturnType<EnvVars['TENANT_D0']['get']>, t_id_hex: string) => {
	const [tenant, { byo_bw }] = await Promise.all([readTenantRow(r_db, t_id_hex), t_do.getProperties({ byo_bw: true })]);

	if (!tenant) return null;
	if (!byo_bw) return { mode: 'managed' as const, tenant, byo_bw: null, project: null, endpoints: null };
	...
	try {
		...
		if (!connection) return { mode: 'managed' as const, tenant, byo_bw: null, project: null, endpoints: null };

		const note = JSON.parse(await stub.decryptSecret(accessToken, connection.note)) as zm.output<typeof TenantByoBwNoteSchema>;
		return { mode: 'bitwarden' as const, tenant, byo_bw: byo_bw as UUID, project: note.project, endpoints: note.endpoints };
	} finally {
...
const useVaultConnection = routeLoader$(({ sharedMap, platform, resolveValue }) => async () => {
	...
	const connection = await readVaultConnection(platform, sharedMap.get('r_db') as DrizzleD1Database, sharedMap.get('t_do') as ReturnType<EnvVars['TENANT_D0']['get']>, sharedMap.get('t_id_hex') as string);
