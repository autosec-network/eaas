...
import { component$, getLocale, Resource, useComputed$, useSignal, useStore, useVisibleTask$ } from '@builder.io/qwik';
...
import { workflowInstanceId } from 'helpers';
...
const useUpdateKeyring = routeAction$(
	async (data, { sharedMap, platform, request, fail }) => {
		...
		/**
		 * `key_type`, `key_size` and `hash` are deliberately absent: they describe key material that already exists in the vault, and changing them here would only mean the next rotation silently generates a key of a different shape than every key before it. `plaintext_export` is write-once for the same reason - it's a security setting the existing datakeys were created under.
		 */
														...
														{row.can_edit ? (
															<Form action={updateKeyring} class="flex flex-wrap items-end gap-3 rounded-xl border border-gray-200 p-3 dark:border-gray-700">
																...
																<label class="inline-flex items-center gap-2 pb-1.5 text-xs text-gray-600 dark:text-gray-300">
																	<input type="checkbox" name="time_rotation" checked={row.time_rotation} />
																...
																<label class="w-44 text-xs text-gray-600 dark:text-gray-300">
																	...
																	<input name="count_rotation" class={fieldClass} inputMode="numeric" pattern="[0-9]*" disabled={!countEnabled} value={row.count_rotation ?? ''} />
																</label>
				...
				<div class="relative max-h-full w-full max-w-3xl p-4">
					<div class="relative rounded-lg bg-white shadow-sm dark:bg-gray-700">
						<div class="flex items-center justify-between rounded-t border-b border-gray-200 p-4 md:p-5 dark:border-gray-600">
							...
							<button type="button" data-modal-hide="create-keyring-modal" class="ms-auto rounded-lg bg-transparent px-2 py-1 text-sm font-medium text-gray-400 hover:bg-gray-200 hover:text-gray-900 dark:hover:bg-gray-600 dark:hover:text-white">
								{m.common_discard()}
							</button>
						</div>
							...
							<div class="space-y-4 p-4 md:p-5">
								...
								<div class="flex flex-wrap gap-3">
									<label class="min-w-48 flex-1 text-sm text-gray-600 dark:text-gray-300">
										...
										<select name="key_type" class={fieldClass} value={createKeyType.value} onChange$={(_, element) => (createKeyType.value = element.value as KeyAlgorithms)}>
									...
									<label class="w-40 text-sm text-gray-600 dark:text-gray-300">
										...
										{createKeySizeRule.value.kind === 'choice' ? (
											<select name="key_size" class={fieldClass} value={String(createKeySizeRule.value.fallback)}>
												{createKeySizeRule.value.options.map((size) => (
													<option key={size} value={String(size)}>
														{String(size)}
													</option>
												))}
											</select>
										) : createKeySizeRule.value.kind === 'range' ? (
											<input name="key_size" type="number" class={fieldClass} min={createKeySizeRule.value.min} max={createKeySizeRule.value.max} step={createKeySizeRule.value.step} value={createKeySizeRule.value.fallback} />
										) : (
									...
									<label class="w-44 text-sm text-gray-600 dark:text-gray-300">
										...
										<select name="hash" class={fieldClass} value={DEFAULT_KEYRING_HASH}>
								...
								<div class="flex flex-wrap items-end gap-3">
									...
									<label class="inline-flex items-center gap-2 pb-1.5 text-sm text-gray-600 dark:text-gray-300">
										<input type="checkbox" name="time_rotation" checked={Boolean(tenantSchema.keyrings.time_rotation.default ?? true)} />
										{m.keyrings_field_time_rotation()}
									</label>
									...
									<label class="w-48 text-sm text-gray-600 dark:text-gray-300">
										{m.keyrings_field_count_rotation_threshold()}
										<input name="count_rotation" class={fieldClass} inputMode="numeric" pattern="[0-9]*" disabled={!createCountRotation.value} value={(BigInt(2) ** BigInt(32)).toString()} />
									</label>
								</div>
