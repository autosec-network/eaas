...
export const onRequest: RequestHandler = async ({ sharedMap, redirect, url, platform, request, locale }) => {
	...
	await (async () => {
		const forceSignIn = async () => {
			...
			throw redirect(307, `/auth/signin?callbackUrl=${url.pathname}`);
		};
	...
	await (async () => {
		...
		if (headers.has('Accept-Language')) {
			...
			platform.env.LANG_ANALYTICS?.writeDataPoint({
				// To be able to sample by session to unskew results by remove duplicates but anonymize (sessions expire faster than this data is needed for)
				indexes: [createHash('sha512').update(session.do_id!).digest('base64')],
				// Top languages in order of priority (caped at 20 due to Analytics Engine limits)
				blobs: parsedLocales.map(({ lang }) => lang).slice(0, 20),
			});
