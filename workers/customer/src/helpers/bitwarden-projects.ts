...
export const getProjects = server$(async function (jurisdiction: DOJurisdictions | null, baseEndpoint: string, authEndpoint: string, apiKey: string) {
	...
	try {
		...
		return chunks.reduce<Promise<{ id: string; name: string }[]>>(async (acc, chunk) => [...(await acc), ...(await Promise.all(chunk.map(async ({ id, name }) => ({ id, name: await doStub.decryptSecret(apiKey, name) }))))], Promise.resolve([]));
	} catch (error) {
