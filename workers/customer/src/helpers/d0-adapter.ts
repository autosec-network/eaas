 ...
 * @param {DrizzleD1Database<typeof rootSchema>} r_db - The root database connection
 ...
 */
export async function deleteSession(platform: QwikCityPlatform, r_db: DrizzleD1Database<typeof rootSchema>, sessionToken: string, returning: boolean) {
...
export function D0Adapter(platform: QwikCityPlatform, sharedMap: Map<string, any>, request: Request, r_db: DrizzleD1Database<typeof rootSchema>, toCache: boolean): Adapter {
	...
	return {
		...
		createSession: async (session) => {
			...
			console.debug('createSession', { lite, normal, sensitive, debug });
		...
		getSessionAndUser: async (sessionToken) => {
			...
			if (platform.env.NODE_ENV !== 'production') console.debug('getSessionAndUser', 'found', Boolean(selectedUserSession));
