...
import { toPermission } from '~/helpers/keyring-permissions';
...
const linkClass = (active: boolean): string[] => ['block rounded-lg px-2.5 py-2 text-sm font-medium transition-colors', active ? 'bg-primary-accent/10 text-primary-accent dark:bg-primary-accent/20' : 'text-gray-600 hover:bg-gray-100 dark:text-gray-300 dark:hover:bg-gray-800/60'];

const TenantNavigation = component$<TenantNavigationProps>(({ tenantId, pathname }) => {
	const permissions = usePermissions();
	const apiKeysPath = `/${tenantId}/api-keys`;
	const keyringsPath = `/${tenantId}/keyrings`;
	const isApiKeysRoute = pathname === apiKeysPath || pathname === `${apiKeysPath}/`;
	const isKeyringsRoute = pathname === keyringsPath || pathname === `${keyringsPath}/`;
	const canReadApiKeys = Boolean(permissions.value && permissions.value.r_apikeys >= Permissions.Read);
	// A `users_keyrings` row can hand over a single keyring to someone whose tenant-wide `r_keyring` is `None`, so the tenant-wide level alone doesn't decide whether this link is worth showing
	const canReadKeyrings = Boolean(permissions.value && (permissions.value.r_keyring >= Permissions.Read || Object.values(permissions.value.overrides).some((override) => toPermission(override['r_keyring']) >= Permissions.Read)));

	if (!canReadApiKeys && !canReadKeyrings) return null;

	return (
		<>
			<p class="px-2 text-xs font-semibold tracking-wide text-gray-500 uppercase dark:text-gray-400">{m.users_permission_tenant()}</p>
			{canReadKeyrings ? (
				<Link prefetch="js" href={keyringsPath} class={linkClass(isKeyringsRoute)}>
					{m.sidebar_group_keyrings()}
				</Link>
			) : null}
			{canReadApiKeys ? (
				<Link prefetch="js" href={apiKeysPath} class={linkClass(isApiKeysRoute)}>
