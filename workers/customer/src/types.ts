...
interface TypedBindings {
	...
	/**
	 * Cross-script binding to the `VaultMigration` workflow defined in the `api` worker. `wrangler types` can only see it as an untyped `Workflow`, so the parameter shape is restated here off the schema both sides share.
	 */
