...
All API error responses **must** use the [RFC 9457 Problem Details](https://www.rfc-editor.org/rfc/rfc9457) format via helpers in `~/errors.mjs`. The `type` URI deep-links into the Scalar API docs at `https://api.eaas.autosec.network/`.

```typescript
import { problemJson, problemJsonValidation, problemResponse } from '~/errors.mjs';

// Return an RFC 9457 error from any route handler
return problemJson(c, 403, { detail: 'Access denied' });

// Include error objects in the response
return problemJson(c, 500, { detail: 'Operation failed', errors: [error] });

// Validation errors (used in defaultHook and zValidator hooks)
return problemJsonValidation(c, result.error);

// With custom status/detail override (e.g. 404 for invalid version)
return problemJsonValidation(c, result.error, 404, "API version doesn't exist");

// OpenAPI response schema shorthand
...
Response body shape (`application/problem+json`):

```json
{
	"type": "https://api.eaas.autosec.network/#tag/api-key-management/DELETE/apikeys/{token_id}",
	"status": 403,
	"title": "Forbidden",
	"detail": "Access Denied: You do not have permission to perform this action",
	"instance": "/v0/apikeys/abc123",
	"errors": [{ "name": "Error", "message": "Access Denied" }]
}
```

Error entry shapes:

- **Normal error**: `{ name: string, message: string, cause?: string }`
- **AggregateError**: `{ message: string, cause?: string, errors: [...] }` (no `name`, has `errors` — applied recursively)
- **Zod errors**: automatically run through `prettifyError()` from `zod/v4`; multiple issues from the same schema become an AggregateError containing individual prettified issues
...
41. **Never `void` async calls in Workers or Qwik SSR** — either `await` the call (when you need its result or must prevent race conditions) or register it with `ctx.waitUntil()` / `event.waitUntil()` to guarantee it completes before the runtime terminates. Fire-and-forget with `void` silently drops the work when the request ends.
42. **Use RFC 9457 Problem Details for all API error responses** — import `problemJson` from `~/errors.mjs` and return errors with `application/problem+json` content type; never use ad-hoc `c.json({ success: false, errors: [...] })` patterns
43. **Use `problemJsonValidation()` for Zod validation errors** — in defaultHook and `zValidator` hooks, call `problemJsonValidation(c, zodError)` to format validation failures; it runs issues through `prettifyError()` and converts multi-issue schemas into AggregateError structure
44. **Use `problemResponse(description)` for OpenAPI error response schemas** — shorthand that sets `application/problem+json` content type and references the shared `ProblemDetails` component
45. **Catch unhandled API errors with `app.onError`** — the global handler in `base.mts` converts thrown errors to RFC 9457 responses; route handlers that `throw` are automatically covered
...
31. **Never `void` async calls in Workers or Qwik SSR** — `void expr` silently discards the promise; the runtime may terminate before the work finishes. Either `await` the call or pass it to `ctx.waitUntil()` / `event.waitUntil()` so the platform keeps the runtime alive until it resolves.
32. **Don't use `c.json({ success: false, errors: [...] })` for API errors** — use RFC 9457 `problemJson()` from `~/errors.mjs`; the old `{ success, errors }` format is retired
33. **Don't create custom error response formats in API routes** — all error responses must use the RFC 9457 Problem Details format with `application/problem+json` content type via the helpers in `~/errors.mjs`
34. **Don't use `application/json` for API error responses** — `problemJson()` automatically sets `application/problem+json`; `problemResponse()` sets it in OpenAPI schemas
